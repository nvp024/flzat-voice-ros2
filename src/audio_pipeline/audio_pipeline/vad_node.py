...
def _default_silero_jit_path() -> str:
    ...
    if colcon_prefixes:
        ...
        # The source workspace lives at <project>/flzat_robot_ws/src/....
        ...
        if is_voice:
            ...
            if self._in_speech:
                ...
                if self._silence_count >= self._silence_chunks:
                    ...
                    self._speech_count  = 0
                    self._silence_count = 0
                    if count >= self._min_speech_chunks:
                        return ("end", frames)
            else:
...
class VadNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._vad_stream = _VADStream(self._vad_model)
        self.get_logger().info("VAD model ready.")
    ...
    def _on_tts_active(self, msg: Bool) -> None:
        if msg.data and not self._tts_active:
            ...
            self._vad_stream.reset()
            self.get_logger().info("TTS inactive — VAD resumed.")
